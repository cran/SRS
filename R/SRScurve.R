    ...
    RSdiversity<-function(data,Cminseq,metric){
      ...
      if (metric=="richness"){  #if the chosen metric was richness, we simply count the number of observed species
        for (i in seq(1,ncol(data),1)){
          j=1
          for (cutoff in Cminseq){
            nspec[i,j]=mean(replicate(length(which(rrarefy(t(data[,i]),sample=cutoff)>0)),n=rarefy.repeats,simplify = "mean"))#repeat the estimate 100 times
            j=j+1
          }
        }
      }
        ...
        for (i in seq(1,ncol(data),1)){
          j=1
          for (cutoff in Cminseq){
            nspec[i,j]=mean(replicate(vegan::diversity(rrarefy(t(data[,i]),sample=cutoff),index=metric),n=100,simplify = "mean"))#repeat the estimate 100 times
            j=j+1
          }
        }
      }
